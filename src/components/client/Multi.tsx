...
export default function Multi({id,title, options, onChangeSelection}:choiceType)  {
            ...
            <div className="my-[8px] mx-[2px] flex flex-col gap-2 lg:mx-[18px]">
                {
                    options.map((choice,index) =>(
                        <div key={`${id}-${index}`} className="flex items-center gap-3 rounded-md bg-white/70 hover:bg-white/90 px-1 py-2 md:py-3 select-none md:px-3" onClick={()=>setVoteState(prev => {
                            const copy = [...prev];
                            copy[index] = !copy[index];
                            onChangeSelection?.(copy
                                .map((v, i) => (v ? i : -1))
                                .filter((i) => i !== -1)
                            );
                            return copy;
                        })}>
                            <input type="checkbox" className="opacity-80 w-[20px] h-[20px] md:w-[32px] md:h-[32px]" checked={voteState[index]} readOnly/>
